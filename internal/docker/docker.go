...
// DiscourseSysctlArgs returns Docker args needed for Discourse containers.
func DiscourseSysctlArgs() []string {
	return []string{"--sysctl", "kernel.unprivileged_userns_clone=1"}
}

func RunDetached(name, workdir, image string, hostPort, containerPort int, labels map[string]string, envs map[string]string, extraHosts []string, sshAuthSock string, extraDockerArgs []string, containerCmd []string, volumes []string) error {
	args := []string{"run", "-d",
		"--init",
		"--name", name,
		"-w", workdir,
